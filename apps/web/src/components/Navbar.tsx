...
import { RefreshCw, Smartphone, Shield, AlertTriangle, CheckCircle, Usb, Laptop } from 'lucide-react';
    ...
    <header className="sticky top-0 z-40 flex h-16 w-full items-center justify-between border-b border-slate-800 bg-slate-950/80 px-6 backdrop-blur-md">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 rounded-lg px-3 py-1.5">
          ...
          <select
            aria-label="Select target Android device"
            className="bg-transparent text-xs font-medium text-slate-200 focus:outline-none cursor-pointer"
            value={selectedDevice?.serial || ''}
            onChange={(e) => {
              const found = devices.find(d => d.serial === e.target.value);
              if (found) onSelectDevice(found);
            }}
          >
            {devices.length === 0 && <option value="">No Devices Detected</option>}
            {devices.map((d) => (
              <option key={d.serial} value={d.serial} className="bg-slate-900 text-slate-200">
                {d.model} ({d.maskedSerial}) {d.isDemo ? '• [DEMO]' : ''}
              </option>
            ))}
          </select>
        </div>

        {/* Live ADB Status Indicator */}
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border text-xs font-semibold tracking-wider uppercase transition-colors bg-slate-900 border-slate-800">
          ...
          {adbStatus?.isInstalled ? (
            <span className="text-emerald-400 flex items-center gap-1.5">
              ...
              READY ({devices.length} {devices.length === 1 ? 'DEV' : 'DEVS'})
            </span>
          ) : (
        ...
        {/* Prominent REFRESH ADB Button */}
        <button
          onClick={fetchStatusAndDevices}
          disabled={isRefreshing}
          className="flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-xs font-bold uppercase tracking-wider text-slate-950 shadow-md shadow-emerald-950 hover:bg-emerald-500 active:scale-95 transition-all disabled:opacity-50"
        >
          <RefreshCw className={`h-3.5 w-3.5 ${isRefreshing ? 'animate-spin' : ''}`} />
          <span>{isRefreshing ? 'SCANNING USB...' : 'REFRESH ADB'}</span>
        </button>
      </div>
    </header>
  );
